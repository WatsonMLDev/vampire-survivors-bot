...
def _get_game_window(process_name_fragment="Vampire"):
    # Heuristic: Find process with "Vampire" in name, then find its main window
    print(f"[Recorder] Searching for process matching '{process_name_fragment}'...")
    pid = None
    
    for proc in psutil.process_iter(['pid', 'name']):
        if process_name_fragment.lower() in proc.info['name'].lower():
            pid = proc.info['pid']
            print(f"[Recorder] Found Process: {proc.info['name']} (PID: {pid})")
            break
            
    if pid is None:
        print(f"[Recorder] WARNING: Process matching '{process_name_fragment}' not found.")
        return None, None
    ...
    if not windows:
        print(f"[Recorder] WARNING: No visible windows for PID {pid}")
        return None, None
...
def _capture_process(stop_event, video_filename, action_filename, fps):
    ...
    try:
        ...
        while pygame.joystick.get_count() == 0 and not stop_event.is_set():
            if retry_count % 5 == 0:
                print("[Recorder] Waiting for controller...")
            time.sleep(1)
        ...
        joystick.init()
        print(f"[Recorder] Connected to joystick: {joystick.get_name()}")

        # Find Window
        hwnd, bbox = _get_game_window("Vampire")
        if not bbox:
            print("[Recorder] Could not find game window. Aborting capture.")
            return

        width = bbox[2] - bbox[0]
        height = bbox[3] - bbox[1]
        print(f"[Recorder] Capture Region: {bbox} ({width}x{height})")
        ...
        if camera is None:
            print("[Recorder] Failed to create DXCAM.")
            return
        ...
        print("[Recorder] Recording started (Process).")
        ...
        print(f"[Recorder] Error in capture process: {e}")
        import traceback
        traceback.print_exc()
    finally:
        print("[Recorder] Cleaning up process...")
...
class Recorder:
    ...
    def start(self):
        ...
        print(f"[Recorder] Starting capture process...")
    ...
    def stop(self):
        ...
        print("[Recorder] Signaling stop to process...")
        self._stop_event.set()
        self._process.join(timeout=5.0)
        if self._process.is_alive():
             print("[Recorder] Process did not exit, terminating...")
             self._process.terminate()
        print("[Recorder] Capture stopped.")
